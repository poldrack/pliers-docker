...
ENV CONDA_DEPS "pip pytest numpy pandas pillow==2.9.0 requests scipy nltk six tqdm seaborn matplotlib pathos tensorflow contextlib2"
WORKDIR /root
...
RUN /bin/bash -c "source /miniconda/envs/py35/bin/activate py35 \
  ...
  && pip install python-magic coveralls pytest-cov pygraphviz pysrt xlrd clarifai pytesseract moviepy==0.2.2.13 SpeechRecognition IndicoIo sklearn python-twitter gensim google-compute-engine librosa ipython"

RUN echo "foo"
ADD pysetup.py /root/pysetup.py
RUN /bin/bash -c 'source /miniconda/envs/py35/bin/activate py35 \
